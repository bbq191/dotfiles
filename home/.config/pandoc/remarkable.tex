...
\newunicodechar{⑤}{\rmglyph{2464}}
\newunicodechar{⑥}{\rmglyph{2465}}
\newunicodechar{★}{\rmglyph{2605}}
\newunicodechar{✕}{\rmsymb{2715}}
\newunicodechar{∪}{\rmglyph{222A}}
\newunicodechar{↔}{\rmglyph{2194}}
\newunicodechar{√}{\rmglyph{221A}}
...
% 通用 emoji 兜底：上面没逐个映射到的 emoji，由 filters.lua 包进 \rmEmoji，
% 用单色 Noto Emoji 渲染（xelatex 渲染不了彩色 emoji 字体）。
% 字体文件：google/fonts 仓库 ofl/notoemoji 的可变字体，默认实例即 Regular。
\newfontfamily\rmEmojiFont{NotoEmoji-Regular.ttf}[Path=\rmfonts, Scale=0.9]
\newcommand{\rmEmoji}[1]{{\rmEmojiFont #1}}
...
% ---------- 中文删除线 ----------
% pandoc 删除线（~~文字~~）默认用 soul 的 \st，参数含 CJK 必报
% "Package soul Error: Reconstruction failed"。换 xeCJKfntef 的 \CJKsout，
% 中英混排都能划线且可断行。延到文档开始覆盖，保证盖过 pandoc 模板的定义。
\usepackage{xeCJKfntef}
\AtBeginDocument{%
  \ifdefined\st\renewcommand{\st}[1]{\CJKsout{#1}}%
  \else\newcommand{\st}[1]{\CJKsout{#1}}\fi}
